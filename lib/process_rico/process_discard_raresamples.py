...
out_dir = "/media/sda2/ljz_dataset/rico_graph_discard_rare_labels"
...
invalid_labels=[23, 22,15,14,11,7]
        ...
        for k, v in json_dict.items():
            if k in label_dict[str(id)].keys():
                ...
                if label_int != -1: # we abandon root node
                    if label_int in invalid_labels:
                        filter_artboard_id.append(str(id))
                        return
                    else:
                        label_int = labels_mapping[label_int]
...
def generate_graph_dataset(dataset_name, max_threads = 4):
    ...
    with open("/media/sda1/ljz-workspace/dataset/clay/split_train_id.txt", "r") as f:
        json.dump([line.strip() for line in f.readlines() if line.strip() not in filter_artboard_id], open(f"{out_dir}/index_train.json", "w"))
    with open("/media/sda1/ljz-workspace/dataset/clay/split_test_id.txt", "r") as f:
        json.dump([line.strip() for line in f.readlines() if line.strip() not in filter_artboard_id], open(f"{out_dir}/index_test.json", "w"))
    with open("/media/sda1/ljz-workspace/dataset/clay/split_dev_id.txt", "r") as f:
        json.dump([line.strip() for line in f.readlines() if line.strip() not in filter_artboard_id], open(f"{out_dir}/index_dev.json", "w"))
