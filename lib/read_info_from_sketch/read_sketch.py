...
class PreProcess:
    ...
    def process(self) -> None:
        ...
        """
        # for each sketch
        for sketch_index, sketch in enumerate(self.sketch_list):

            try:
                if sketch is not None:
                    print(f"processing sketch {sketch}")
                    PreProcessEachSketch(self.outDir,
                        sketch, self.image_list[sketch_index], self.scale).process()
            except Exception as e:
                if sketch is not None:
...
class PreProcessEachSketch:
    def __init__(self, outDir:str, sketch_path: str, sketch_name: str, scale: int):
        self.outDir=outDir
        ...
        for page in self.sketch_file.contents.document.pages:
            ...
            for layer in reduced_page.layers:
                ...
                if isinstance(layer, Artboard):
                    ...
                    self.traverse(layer, Point(0, 0), handler, leafs)
                    print("*"*80, len(leafs))
                    handler.finish(self.sketch_path)
